...
public class StateMachine : MonoBehaviour
{
    ...
    private bool UpdateThisEvent(SMEvent smEvent)
    {
        if (false == _callbacks.ContainsKey(string.Format("onupdate{0}", smEvent.Name)))
            return true;
        return DoCallback(_callbacks[string.Format("onupdate{0}", smEvent.Name)], smEvent);
    }
